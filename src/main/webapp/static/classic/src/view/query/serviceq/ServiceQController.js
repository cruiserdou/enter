 ...
 */
Ext.define('app.view.query.serviceq.ServiceQController', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.serviceqcontroller',
    requires: [
    ...
    refresh: function () {
        Ext.getCmp('serviceqgridview_id').getStore().load();
    },

    btnFind: function(){
        Ext.getCmp('serviceqgridview_id').getStore().load({
            params: {
                name: Ext.getCmp('query_ser_q_name_id').getValue(),
                nos: Ext.getCmp('query_ser_q_nos_id').getValue(),
                buslicno: Ext.getCmp('query_ser_q_buslicno_id').getValue(),
                listcode: Ext.getCmp('query_ser_q_listcode_id').getValue()
    ...
    btnSearch: function (_this) {
        if (!Ext.getCmp('serviceqqueryview_id')) {
            _this.up().up().add(
                {
                    xtype: 'serviceqqueryview',
                    id: 'serviceqqueryview_id',
                    region: 'north',
                    margin: '1 0 0 0'
                }
            );
        }else{
            _this.up().up().remove(Ext.getCmp('serviceqqueryview_id'));
        }
    },
    ...
    btnLog: function (_this) {
        if (!Ext.getCmp('corpservereditloggridview_id')) {
            _this.up().up().add(
                {
                    xtype: 'corpservereditloggridview',
                    id: 'corpservereditloggridview_id',
                    title: '变更记录',
                    collapsible: true,
                    region: 'south',
                    margin: '1 0 0 0',
            ...
            _this.up().up().remove(Ext.getCmp('corpservereditloggridview_id'));
        }
    },
    ...
    delete: function () {
        Ext.Msg.confirm('信息', '确定要删除所选信息吗？', function (btn) {
            if (btn == 'yes') {
                var sm = Ext.getCmp('serviceqgridview_id').getSelectionModel();
