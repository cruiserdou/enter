 ...
 */
Ext.define('app.view.maintain.servicemt.ServiceMtController', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.servicemtcontroller',
    requires: [
        'app.xtemplate.corp_contact'
    ],
    itemclick: function (this_, record_) {
        var vPanel = Ext.getCmp('servicemtdetailview_id');
        ...
        var mypanel = new Ext.form.FormPanel({
            ...
            listeners: {
                afterrender: function (_this){
                    ...
                    corp_acount_tpl.append('service_manage_corp_acount',record.data);
                    corp_maintain_tpl.append('service_manage_corp_maintain',record.data);
                    corp_finance_tpl.append('service_manage_corp_finance',record.data);
                    corp_service_con_tpl.append('service_manage_corp_service',record.data);
            ...
            items: [
                {
                    ...
                    border: false,
                    id: 'corp_panel',
                    html: '<div id="service_manage_corp">' +
                    '</div>'
                },
                //{
                //    xtype: 'panel',
                ...
                //},
                {
                    ...
                    html: '<div id="service_manage_corp_contact">' +
                    '</div>'
                },
                {
                    xtype: 'panel',
                    ...
                    html: '<div id="service_manage_corp_acount">' +
                    '</div>'
                },
                {
                    xtype: 'panel',
                    ...
                    html: '<div id="service_manage_corp_maintain">' +
                    '</div>'
                },
                {
                    xtype: 'panel',
    ...
    refresh: function () {
        Ext.getCmp('servicemtgridview_id').getStore().load();
    },

    btnFind: function(){
        Ext.getCmp('servicemtgridview_id').getStore().load({
            params: {
                name: Ext.getCmp('query_ser_mt_name_id').getValue(),
                nos: Ext.getCmp('query_ser_mt_nos_id').getValue(),
                buslicno: Ext.getCmp('query_ser_mt_buslicno_id').getValue(),
                listcode: Ext.getCmp('query_ser_mt_listcode_id').getValue()
    ...
    btnSearch: function (_this) {
        if (!Ext.getCmp('servicemtqueryview_id')) {
            _this.up().up().add(
                {
                    xtype: 'servicemtqueryview',
                    id: 'servicemtqueryview_id',
                    region: 'north',
                    margin: '1 0 0 0'
                }
            );
        }else{
            _this.up().up().remove(Ext.getCmp('servicemtqueryview_id'));
        }
    },
    ...
    delete: function () {
        Ext.Msg.confirm('信息', '确定要删除所选信息吗？', function (btn) {
            if (btn == 'yes') {
                var sm = Ext.getCmp('servicemtgridview_id').getSelectionModel();
